...
function ColumnBarControl({
  ...
  if (column === "empleado" || column === "tipo" || column === "estado") {
    const values =
      existing &&
...
export function AprobacionLegalizacionesFilterBar({
  ...
  const [autoOpenColumn, setAutoOpenColumn] =
    useState<AproLegFilterColumn | null>(null);
  ...
  const pickColumn = (col: AproLegFilterColumn) => {
    setColumnMenuOpen(false);
    setBarColumns((prev) => (prev.includes(col) ? prev : [...prev, col]));
    setAutoOpenColumn(col);
  };

  const removeBarColumn = (col: AproLegFilterColumn) => {
