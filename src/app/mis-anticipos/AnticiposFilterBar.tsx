...
export function AnticiposFilterBar({
  ...
  const [barColumns, setBarColumns] = useState<AnticipoFilterColumn[]>([]);
  const [autoOpenColumn, setAutoOpenColumn] = useState<AnticipoFilterColumn | null>(
    null,
  );
  ...
  const pickColumn = (col: AnticipoFilterColumn) => {
    setColumnMenuOpen(false);
    setBarColumns((prev) => (prev.includes(col) ? prev : [...prev, col]));
    setAutoOpenColumn(col);
  };

  const removeBarColumn = (col: AnticipoFilterColumn) => {
