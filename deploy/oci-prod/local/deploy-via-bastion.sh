...
ENV_FILE="${SCRIPT_DIR}/oci-prod.env"
MODE="${1:-deploy}"
...
Copia oci-prod.env.example → oci-prod.env y llena los OCIDs (una sola vez)."
...
[[ "${OCI_BASTION_OCID}" == ocid1.bastion.* ]] || die "OCI_BASTION_OCID inválido en oci-prod.env"
[[ "${OCI_INSTANCE_OCID}" == ocid1.instance.* ]] || die "OCI_INSTANCE_OCID inválido en oci-prod.env"

SESSION_NAME="portal-prod-$(date +%Y%m%d%H%M%S)"
SESSION_OCID=""
CLEANED=0
...
if [[ "${MODE}" == "restart" ]]; then
  ...
  info "Listo → https://hmv-empleados.nubeportal.com/login"
  exit 0
fi
...
if [[ "${MODE}" == "tunnel" || "${MODE}" == "tunnel-http" ]]; then
  ...
  info "También: https://hmv-empleados.nubeportal.com/login"
  ssh -i "${OCI_SSH_KEY}" \
    -o IdentitiesOnly=yes \
    -o StrictHostKeyChecking=accept-new \
    -o ServerAliveInterval=15 \
...
if ! sudo -u portalnext test -d "${APP_HOME}/.git"; then
  echo "No hay .git en ${APP_HOME}. Corre el bootstrap de la VM PROD una vez."
  exit 1
fi
...
# Callback público PROD (nunca el de DEV ni localhost).
PROD_REDIRECT='https://hmv-empleados.nubeportal.com/api/auth/callback/ifs'
if sudo -u portalnext grep -q '^IFS_OAUTH_REDIRECT_URI=' "${ENVF}"; then
  sudo -u portalnext sed -i "s|^IFS_OAUTH_REDIRECT_URI=.*|IFS_OAUTH_REDIRECT_URI=${PROD_REDIRECT}|" "${ENVF}"
else
  echo "IFS_OAUTH_REDIRECT_URI=${PROD_REDIRECT}" | sudo -u portalnext tee -a "${ENVF}" >/dev/null
fi
